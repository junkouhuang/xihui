...
function inline(str1, str2, fillWith = ' ', fontWidth = 1) {
  ...
  // 需要填充的字符数量
  let fillCount = lineWidth - (getStringWidth(str1) + getStringWidth(str2)) % 6;
  let fillStr = new Array(fillCount).fill(fillWith.charAt(0)).join('');
  return str1 + fillStr + str2;
}


function inline_spmc(str1, str2, fillWith = ' ', fontWidth = 1) {
  const lineWidth = MAX_CHAR_COUNT_EACH_LINE / fontWidth;
  // 需要填充的字符数量
...
function inline_max(str1, str2, str3, str4, str5, str6, str7, fillWith = ' ', fontWidth = 1) {
  const lineWidth = MAX_CHAR_COUNT_EACH_LINE / fontWidth;
  // 需要填充的字符数量
  let fillCount = (lineWidth - (getStringWidth(str1) + getStringWidth(str2) + getStringWidth(str3) + getStringWidth(str4) + getStringWidth(str5) + getStringWidth(str6) + getStringWidth(str7)) )/ 6;
  let fillStr = new Array(2).fill(fillWith.charAt(0)).join('');
  return str1 + fillStr + str2 + fillStr + str3 + fillStr + str4 + fillStr + str5 + fillStr + str6 + fillStr + str7;
}

function inline_min(str1, str2, str3, str4, str5, str6, str7, fillWith = ' ', fontWidth = 1) {
  const lineWidth = MAX_CHAR_COUNT_EACH_LINE / fontWidth;
  // 需要填充的字符数量
  let fillCount = (lineWidth - (getStringWidth(str1) + getStringWidth(str2) + getStringWidth(str3) + getStringWidth(str4) + getStringWidth(str5) + getStringWidth(str6) + getStringWidth(str7))) / 6;
  let fillStr = new Array(5).fill(fillWith.charAt(0)).join('');
  return str1 + fillStr + str2 + fillStr + str3 + fillStr + str4 + fillStr + str5 + fillStr + str6 + fillStr + str7;
}
