...
printerJobs.prototype.print_max = function (obj) {
  let data=new Array();
  data = JSON.parse(obj);
  for (let i = 0; i < data.length;i++){
    this.text(printerUtil.inline_spmc(data[i].spmc + "(" + data[i].unicode+")",""));
    this.setAlign("RT");
    this.text(printerUtil.inline_min('', data[i].ys, data[i].cm,data[i].sellprice, data[i].realprice, data[i].sl, data[i].sl * data[i].realprice));
  }
  this._enqueue(commands.LF);
  return this;
};
